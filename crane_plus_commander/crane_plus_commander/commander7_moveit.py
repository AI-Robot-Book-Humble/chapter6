...
class CommanderMoveit(Node):
    ...
    def pickup(self, words, result):
        ...
        # グリッパを開く
        gripper_ratio = 0.0
        success = self.move_gripper(from_gripper_ratio(gripper_ratio))
        if not success:
            result.answer = 'NG failed to open gripper'
            return
        # 準備姿勢へ動く
        offset = 0.05
        d = sqrt(x2**2 + y2**2)
        ...
        y1 = y2 - offset * y2 / d
        z1 = z2
        success = self.move_endtip([x1, y1, z1, pitch])
        if not success:
            result.answer = 'NG failed to move for the 1st point'
            return
        # 把持姿勢へ動く
        success = self.move_endtip([x2, y2, z2, pitch])
        if not success:
            result.answer = 'NG failed to move for the 2nd point'
            return
        # グリッパを閉じる
        gripper_ratio = 0.95
        success = self.move_gripper(from_gripper_ratio(gripper_ratio))
        if not success:
            result.answer = 'NG failed to close gripper'
            return
        # 運搬姿勢へ動く
        success = self.move_joint(self.poses['carry'])
        if not success:
            result.answer = 'NG failed to move for carry pose'
            return
        result.answer = 'OK'
